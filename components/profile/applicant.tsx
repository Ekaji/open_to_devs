...
export default async function Applicant() {
  ...
  //  await getApplicant(id, 'api/auth/user', "POST") : 
  //  role === 'MODERATOR' || 'ADMIN' || 'EMPLOYER' ?  
  //  await getApplicant(id, 'api/auth/employer', "POST") :  
  //  await getApplicant(id, 'api/auth/user', "POST")

  const response = await getApplicant(id, 'api/auth/user/READ', "POST")
        ...
        <div className="md:flex no-wrap md:-mx-2 ">
            <div className="w-full md:w-3/12 md:mx-2">
                <div className="bg-white p-3 border-t-4 border-green-400">
                    ...
                    <h1 className="text-gray-900 font-bold text-xl leading-8 my-1">{response?.feedbacks?.firstName} {response?.feedbacks?.lastName}</h1>
                    {/* <h3 className="text-gray-600 font-lg text-semibold leading-6">{response?.feedbacks?.designation}</h3> */}
