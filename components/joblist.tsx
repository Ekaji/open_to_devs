...
export default function JobList() {
  ...
  function convertToArray(inputString) {
    ...
    var trimmedString = inputString.slice(1, -1);

    // Remove commas
    var stringWithoutCommas = trimmedString.replace(/,/g, '');

    // Split the string by spaces
    var splitArray = stringWithoutCommas.split(' ');

    // Remove the double quotes from each element
    var finalArray = splitArray.map(function(element) {
...

     {
       response ? (
        ...
        // </div>
            ...
            <a href={`/job/${job.id}`}>
                <div className="px-4 py-3 w- 3/12">
                   <span className="text-[#BEADFA] font-bold mr-3 uppercase text-xs">{job.level}</span>
                   <div>

                   <p className="text-lg font-bold text-black truncate block capitalize">{job.title}</p>
                    <div className="flex flex-col">
                     <p className="text-sm font-semibold text-black cursor-auto my-3">{ job.employer.company_name }</p>
                     <div className="flex text-sm text-gray-600 cursor-auto space-x-2">{
                       convertToArray(job.tags).map(tag => (<p className=" bg-[#BEADFA]/90 p-2 rounded-sm"> {tag}</p>))
                     }</div>
                    </div>
                   </div>
                </div>
